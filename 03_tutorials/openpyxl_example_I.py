...
from mp_data import find_last_row
...
found_max_row=find_last_row(xls_book,0,2,3)
...
for col_num in range(4,13,2): #using the range function with the step method -> important_cols=[4,6,8,10,12]
    ...
    for row_index, row in enumerate(data_sheet.iter_rows(min_row=2,max_row=found_max_row,min_col=col_num,max_col=col_num)):
        for cell in row:
            
            #check previous value is false and current one is true -> start of a package on a  working station
            if (cell.value=="=TRIM(\"true\")" or cell.value=="=TRIM(\" true\")"):
                if (previous_cell_value=="=TRIM(\" false\")" or previous_cell_value=="=TRIM(\"false\")"): #algorithm
    ...
    print("transfering for {} is done".format(col_num))
